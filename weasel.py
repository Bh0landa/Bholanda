#1. Generate a random phrase with the same length as the target phrase.
#2. Create 100 copies of this phrase.
#3. Apply mutations to each character in the copies, with a 5% chance of changing to another random character.
#4. Score each copy by comparing with the target phrase, counting how many characters are correct and in the right position.
#5. Select the best copy (highest score) to be the base for the next generation.
#6. Repeat the process until one of the copies matches the target phrase.
...
def random_phrase(length):
    # Step 1: Generate a random phrase with the same length as the target phrase
    return ''.join(random_character() for _ in range(length))

def mutate(phrase):
    # Step 3: Apply mutations to each character in the phrase, with a 5% chance
    return ''.join(
...
def main():
    ...
    while True:
        ...
        scores = [score(f) for f in population]
